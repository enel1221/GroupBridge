...
`EventListenerProvider` that turns successful group and membership admin events, plus
explicitly allowlisted OIDC client logins, into authenticated reconciliation hints.
GroupBridge always reads current state back from Keycloak; the webhook is intentionally
not a durable event log.

The provider compiles against Keycloak **26.6.4** and Java 21. Its JAR contains no
runtime libraries: it uses Keycloak's supplied SPI and the JDK HTTP and cryptography
APIs. CI starts an aligned runtime, enables the listener, creates a group, and proves
that the same JAR emits a committed event hint on these exact releases:

- 26.0.8, 26.1.5, 26.2.5, 26.3.5;
- 26.4.7, 26.5.7, 26.6.2, 26.6.4; and
- 26.7.0.

This is an exact-version test matrix, not a blanket guarantee for every 26.x patch.
Keycloak labels the documented event-listener SPI as internal, so its
`KC-SERVICES0047` startup warning is expected. Test every new Keycloak version before
deployment and keep the Keycloak runtime image version explicit.

Accordingly, the build has a `provided` dependency on
`keycloak-server-spi-private`. The tested Keycloak releases place `EventListenerProvider`,
...
- a path shaped like `users/{user-id}/groups/{group-id}` when Keycloak reports its
  resource type as `USER`;
- direct `USER` update/delete events, so disable and deletion revoke prior group access
  within seconds; and
- successful `LOGIN` events only when the event's client ID is in the configured
  `jit-client-ids` allowlist, to close the selected GitLab OIDC JIT race.

It ignores registration, non-allowlisted login, unrelated, and failed events. The
allowlist defaults to empty so ordinary realm login traffic cannot trigger full
snapshots. An immutable, metadata-only hint is queued only after Keycloak's transaction
commits. Network delivery runs on a small daemon worker pool, never on the Keycloak
request thread. There are no retries: a periodic GroupBridge full reconciliation is the
recovery mechanism.
...
the webhook secret are never added to the payload or logs. Keycloak group IDs, user IDs,
realm IDs, and raw admin resource paths are also omitted. `groupKey` and `userKey` are
domain-separated HMAC-SHA-256 routing keys derived from the shared webhook secret, realm
name, and one native identifier. They are opaque scheduling keys, never desired state.

Allowlisted login hints use the same envelope with `resourceType: "USER"` and
`operationType: "LOGIN"` and only a `userKey`. Direct USER update/delete uses the same
private `userKey`; membership events use only `groupKey`. Client ID, native user/group
ID, username, session ID, IP address, and event details are all omitted.
...
CI publishes that scratch build-context artifact as
`ghcr.io/enel1221/groupbridge-keycloak-extension:<immutable-tag>`. It has no executable
or shell and is not an init container. Consume it only in a trusted image build.
Versioned, pre-augmented runtime images are published for current deployment lines as
`ghcr.io/enel1221/groupbridge-keycloak:<immutable-tag>-keycloak-<version>`. Always pin
the selected runtime by digest.
...
| `KC_SPI_EVENTS_LISTENER__GROUPBRIDGE__JIT_CLIENT_IDS` | `--spi-events-listener--groupbridge--jit-client-ids` | empty; comma-separated exact client IDs |

Keycloak 26.0 uses the legacy single-underscore environment spelling
`KC_SPI_EVENTS_LISTENER_GROUPBRIDGE_<OPTION>`. The compatibility smoke test supplies
both spellings with identical values. Use the double-underscore form shown above on
current Keycloak releases.
...
The listener uses Keycloak's JVM default trust configuration. For an internal CA,
mount the CA and configure Keycloak's supported `KC_TRUSTSTORE_PATHS`; never disable
certificate or hostname verification. The webhook sender refuses redirects.
...
- GroupBridge coalesces a burst for a 200 millisecond quiet period with a two-second
  maximum delay, then authoritatively re-reads only known affected groups. Unknown
  routing keys request a rate-limited full repair. Allowlisted login hints schedule
  keyed three-second OIDC JIT retries, and revocation hints schedule a second exact
  membership read after 1.5 seconds.
